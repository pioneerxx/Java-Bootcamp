...
class Program {
    public static void main(String args[]) {
        ...
        BufferedImage image;
        try {
            if (args.length != 3 || !args[0].startsWith("--black=") 
            || !args[1].startsWith("--white=")
            || !args[2].startsWith("--imagePath=")
            || args[0].split("=")[1].length() != 1
            || args[1].split("=")[1].length() != 1
            || !args[2].endsWith(".bmp")
            || !args[2].split("=")[1].startsWith("/")) {
                System.err.println("Wrong input");
                System.err.println("Usage: java Java.edu.school.printer.app.Program --black=*char* --white=*char* --imagePath=*path.bmp*");
                System.exit(-1);
            }
            black = args[0].split("=")[1];
            white = args[1].split("=")[1];
            image = ImageIO.read(new File(args[2].split("=")[1]));
